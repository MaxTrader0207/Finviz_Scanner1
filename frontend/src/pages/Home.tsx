...
// 「產業構成」長條圖原本用 recharts 畫，但它是主打包檔裡最大、且每個畫面
// 一載入就會顯示、無法延遲載入的一塊依賴。這種簡單的水平長條圖用純 CSS 就能
// 做到一樣的視覺效果，改成下面的 SectorCompositionChart，把整個 recharts
// 套件從打包結果裡拿掉。
...
/**
 * 純 CSS 的水平長條圖，取代原本的 recharts 實作。維持一樣的視覺呈現：
 * 第一名（index 0）用主色（teal），其餘用次色（copper），長條寬度依 count/axisMax
 * 等比例縮放，右側顯示百分比文字。不需要 ResponsiveContainer 或任何 JS 算圖表
 * 高度——每一列的高度就是它實際需要的高度（產業名稱換行也會自然撐高），
 * 不會再發生「類別一多，標籤被自動隱藏」的問題。
 */
function SectorCompositionChart({ data, axisMax }: { data: Array<{ name: string; count: number; percentageLabel: string }>; axisMax: number }) {
  return (
    <div className="sector-chart-native" role="img" aria-label="產業構成長條圖">
      {data.map((item, index) => (
        <div className="sector-chart-row" key={item.name}>
          <span className="sector-chart-label">{item.name}</span>
          <div className="sector-chart-track">
            <div className={`sector-chart-bar${index === 0 ? " is-leading" : ""}`} style={{ width: `${axisMax > 0 ? Math.min(100, (item.count / axisMax) * 100) : 0}%` }} />
          </div>
          <span className="sector-chart-value">{item.percentageLabel}</span>
        </div>
      ))}
    </div>
  );
...
export default function Home() {
  ...
  const toggleWatchlist = useCallback((stock: Stock | string, source?: string) => {
  ...
  const requestSummary = useCallback((stock: Stock) => {
  ...
  // StockRow 包了 React.memo，但如果每次渲染都傳進去一個全新的箭頭函式當
  // callback prop，memo 的 props 比較永遠會判定「有變化」而失去效果。這三個
  // 用 useCallback 包起來，讓同一份函式參照在不相關的重新渲染之間保持不變
  // （只有真正相關的依賴改變時才會換新的參照），畫面上其餘沒變的股票列才能
  // 真的被 memo 跳過、不必整批重新渲染——這在手機較弱的處理器上對操作流暢度
  // 影響尤其明顯。
  const handleToggleExpand = useCallback((ticker: string) => {
    setExpandedTicker((current) => (current === ticker ? null : ticker));
  }, []);

  const handleToggleWatchlistRow = useCallback(
    (stock: Stock) => toggleWatchlist(stock, screen.shortName),
    [toggleWatchlist, screen.shortName]
  );
      ...
      <main id="top" className="data-stage">
        ...
        <section className="analysis-grid">
          ...
          <article className="sector-panel">
            ...
            {sectorData.length ? <SectorCompositionChart data={sectorData} axisMax={sectorAxisMax} /> : <div className="sector-empty"><Layers size={19} /><p>本次快照無符合結果</p></div>}
          </article>
        </section>
        ...
        <section id="watchlist-panel" className={`watchlist-panel watchlist-bottom ${isWatchlistOpen ? "is-open" : ""}`} aria-label="自選股清單">
          ...
          {isWatchlistOpen && <div id="watchlist-content" className="watchlist-content">{favoriteStocks.length ? <>
            ...
            <section className="watchlist-sector-panel sector-panel" aria-label="自選股產業構成">
              ...
              <SectorCompositionChart data={watchlistSectorData} axisMax={watchlistSectorAxisMax} />
            </section>
...
const StockRow = memo(function StockRow({ stock, rank, history, currentPrice, isHistoryLoading, isExpanded, isFavorite, onToggle, onToggleWatchlist, onSummary }: { stock: Stock; rank: number; history?: MiniPriceHistory; currentPrice?: number; isHistoryLoading: boolean; isExpanded: boolean; isFavorite: boolean; onToggle: (ticker: string) => void; onToggleWatchlist: (stock: Stock) => void; onSummary: (stock: Stock) => void }) {
